"""Sandbox configuration for secure Python code execution."""
import os
...
SANDBOX_CONFIG = {
    ...
    # CPU limit (percentage of single core)
    "cpu_limit_percent": 50,

    # Disable network access
    "network_enabled": False,

    # Disable filesystem access (except /tmp)
    "filesystem_enabled": False,

    # Allowed /tmp size in MB
    "tmp_size_mb": 10,
...
def get_docker_run_command(code_file_path: str) -> list:
    """
    Generate Docker run command with security constraints.

    Args:
        code_file_path: Path to Python file to execute
    ...
    return [
        ...
        "--memory", f"{SANDBOX_CONFIG['memory_limit_mb']}m",  # Memory limit
        "--memory-swap", f"{SANDBOX_CONFIG['memory_limit_mb']}m",  # No swap
        "--cpus", str(SANDBOX_CONFIG['cpu_limit_percent'] / 100),  # CPU limit
        "--pids-limit", "50",  # Limit number of processes
        "--read-only",  # Read-only filesystem
        "--tmpfs", f"/tmp:size={SANDBOX_CONFIG['tmp_size_mb']}m",  # Temporary filesystem
        "--security-opt", "no-new-privileges",  # Prevent privilege escalation
        "--cap-drop", "ALL",  # Drop all capabilities
        "-v", f"{code_file_path}:/code.py:ro",  # Mount code file read-only
        SANDBOX_CONFIG['docker_image'],
        "python3", "/code.py"
    ]

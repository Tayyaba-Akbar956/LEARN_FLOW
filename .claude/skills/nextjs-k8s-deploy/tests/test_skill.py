#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Real execution tests for nextjs-k8s-deploy skill"""
import os
import sys
import subprocess
...
def test_build_script_syntax():
    ...
    result = subprocess.run(
        ['bash', '-n', '.claude/skills/nextjs-k8s-deploy/scripts/build.sh'],
...
def test_load_image_script_syntax():
    """Test that load_image.sh has valid bash syntax"""
    result = subprocess.run(
        ['bash', '-n', '.claude/skills/nextjs-k8s-deploy/scripts/load_image.sh'],
        capture_output=True, text=True
    )

    if result.returncode != 0:
        print(f"✗ FAIL: load_image.sh has syntax errors: {result.stderr}")
        return False

    print("✓ PASS: load_image.sh has valid bash syntax")
    return True

def test_deploy_script_syntax():
    """Test that deploy.sh has valid bash syntax"""
    result = subprocess.run(
        ['bash', '-n', '.claude/skills/nextjs-k8s-deploy/scripts/deploy.sh'],
        capture_output=True, text=True
    )
...
def test_deploy_uses_inline_yaml():
    """Test that deploy.sh actually uses inline YAML (not external files)"""
    script_path = '.claude/skills/nextjs-k8s-deploy/scripts/deploy.sh'

    with open(script_path, 'r', encoding='utf-8') as f:
        content = f.read()

    # Check for inline YAML pattern
...
def test_deployment_uses_never_pull_policy():
    """Test that deployment actually uses imagePullPolicy: Never"""
    script_path = '.claude/skills/nextjs-k8s-deploy/scripts/deploy.sh'
...
def test_scripts_have_error_handling():
    ...
    scripts = [
        '.claude/skills/nextjs-k8s-deploy/scripts/build.sh',
        '.claude/skills/nextjs-k8s-deploy/scripts/load_image.sh',
